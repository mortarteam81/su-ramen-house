...
async function makeFirstBowl(page) {
  ...
  await waitForState(page, (s) => s.selectedPot === 0 && s.guidance.guideTargets.includes('water'), 'water guidance');
  ...
  await waitForState(page, (s) => s.pots[0].state === 'cooking' && s.pots[0].targetRecipe === 'basic', 'basic cooking');
  await advanceTime(page, 5100);
  await waitForState(page, (s) => s.pots[0].state === 'done', 'basic bowl done');
